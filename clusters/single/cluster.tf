...
#            "sudo umount /dev/xvdd; sudo mkswap /dev/xvdd; sudo swapon /dev/xvdd",
    ...
    #    command = "echo ${join('\n', aws_instance.member.*.public_ip)} >> pub_ip.txt; echo ${join('\n', aws_instance.member.*.private_ip)} >> pri_ip.txt;"
    #}
}
...
#            "sudo yum -y update",
            ...
            "wget --quiet --no-check-certificate http://central.maven.org/maven2/org/mongodb/mongo-java-driver/2.12.5/mongo-java-driver-2.12.5.jar",
